...
<x-app-layout>
    ...
    <div class="card-body">
        ...
        {{ csrf_field() }}
            ...
            @foreach($data as $row)
            <tr class="row{{$row->id}}">
                <td class="text-center">{{GeneralHelper::getSupplierName($row -> supplier)}}</td>
                <td class="text-center">{{$row -> name}}</td>
                <td class="text-center">{{$row -> quantity}}</td>
                <td class="text-center">{{$row -> date}}</td>
                <td class="text-center"><a  class="btn btn-secondary" href="{{route('asset.edit',$row->id)}}">Edit</a></td>
                <td class="text-center"><a class="btn btn-warning" href="{{ route('asset.show', $row->id) }}">Print</a></td>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-text-style">
            ...
            <div class="modal-footer">
                ...
                <form  action="{{route('asset.destroy','null')}}" method="post">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
...
//     var RequestUrl = BaseUrl+"/asset/"+id+"/edit";
